...
/* 				-webkit-transition-property: -webkit-filter; */
	...
	<body style="margin: 0;" onScroll="blurPhoto();">
		...
		$questions = array_map('str_getcsv', file('../bartquestions.csv'));
		...
		<div id="content_wrapper_verytop" style="background-color:#f4f5f0;">
			...
			foreach($candidate as &$question) {
				if($curQuestion >= $firstQuestion) {
					if($questions[0][$curQuestion-2] != "") {
					$curNum++;
					?>
			<div id="content_wrapper_sub" style="background-color:<?php echo $backgrounds[$curNum % 2];?>;">
				...
				if($curQuestion == $firstQuestion) {
					echo $candidate["member"] . ". " . $candidate["memberwhy"];
				} else {
						echo $question; 
				}
